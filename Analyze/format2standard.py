...
class Format2Standard:
    ...
    def select_device(self):
        device_list = ['A100','4090','Jetson Xanier NX','MR-100','BI-150','MLU370-M8','910B', '910A', 'M2-Ultra']
        print("\n---------------------------------------------------------------------")
    ...
    def convert(self):
        ...
        if (self.device in ['M2-Ultra']):
            with open(self.input_path, 'r') as f:
                data = f.readlines()
            baseline = None
            simple = data[0]
            Total_memory = int(float(simple.split("'Total Memory (bytes)': ")[1].split(",")[0].replace("'", "")))   # Bytes
            base_Power_Usage = int(float(simple.split("'Power Usage (mW)': ")[1].split("}")[0].replace("'", "")))
            
            # line : "1723106336.666979: {'GPU Utilization (%)': '98', 'Memory Utilization (%)': 1.715087890625, 'Total Memory (MiB)': '32768', 'Used Memory (MiB)': '562', 'Free Memory (MiB)': '32206', 'Power Usage (W)': '102', 'Temperature (C)': '38'}"        
            start_time = float(data[0].split(':')[0])
            for line in data[0:]:
                try:
                    time_now = float(line.split(':')[0]) - start_time
                    GPU_Utilization = int(line.split("'GPU Utilization (%)': ")[1].split(",")[0].replace("'", ""))
                    Used_Memory = int(float(line.split("'Used Memory (bytes)': ")[1].split(",")[0].replace("'", "")))              # Bytes
                    Memory_Utilization = 100 * (Used_Memory / Total_memory)
                    Power_Usage = int(float(simple.split("'Power Usage (mW)': ")[1].split("}")[0].replace("'", "")) * 1000)   # W -> mW
                    Temperature = -1                                                                                          # 无温度数据
                    temp_data.append([time_now, GPU_Utilization, Memory_Utilization, Used_Memory, Power_Usage, Temperature])
                except:
                    break
